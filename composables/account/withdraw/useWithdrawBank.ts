export const useWithdrawBank = () => {
  ...
  const { mappedUserBankList } = storeToRefs(appStore);
  const { currentUser } = useCurrentUser();

  const { refreshUserInfo } = useRefresh();
  const { createToast } = useToast();
  const { setComposableResponse } = useSharedComposableResponse<IWithdrawBankData>();
  ...
  const submitFormWithdrawBank = async (): Promise<void> => {
    try {
      const response = await $withdrawService.getWithdrawBank(formWithdrawBank);
