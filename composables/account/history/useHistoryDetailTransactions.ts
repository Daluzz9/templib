  ...
  const getTransactionsDetail = async (id: string) => {
    try {
      ...
      if (!isEmpty(data)) {
        ...
        dataCardSerial.value =
          (data as IPhoneCardInvoice)?.card_serial?.toString()?.trim()[0] === "[" ||
          (data as IPhoneCardInvoice)?.card_serial?.toString()?.trim()[0] === "{"
            ? JSON.parse((data as IPhoneCardInvoice).card_serial.toString())
            : (data as IPhoneCardInvoice)?.card_serial?.toString() || "";
