export default function useAccountMenu() {
  ...
  const depositCryptoMenu = computed(() => {
    const isUnderMaintenance = !(
      systemDepositPayment.value?.crypto_v2 && systemDepositPayment.value.crypto_v2.length > 0
    );
  ...
  const maintenanceMethodMap = computed(() => ({
    ...
    [AccountDepositMethodEnum.CRYPTO]: depositCryptoMenu.value,
  ...
  const menuAccountDepositMobile = computed(() => {
    return accountDepositMethodMenusMobile.map((menu) => {
      return {
        ...menu,
        isUnderMaintenance:
          [...depositBankMenus.value, ...menuAccountDeposit.value].find((item) => item.path === menu.path)
