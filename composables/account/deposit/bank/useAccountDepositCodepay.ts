...
export default function useAccountDepositCodepay() {
  ...
  const { isUserBankAvailable, mappedPackages: promotionPackages, isCommissionPackage } = storeToRefs(appStore);
  ...
  const detailDeposit = reactive<IDepositCodePayData & { timeLeft?: Date; timeLeftCreateNewInvoice?: Date }>({
    invoice_id: 0,
    amount: 0,
    code: "",
    bank_code: "",
