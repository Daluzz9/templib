...
export const useFetchGames = (
  ...
  const stateKeys = {
    ...
    totalLobbyGame: keyGenerator("total_game_lobby"),
  };
  ...
  const totalLobbyGame = useState<number>(stateKeys.totalLobbyGame, () => 0);
  ...
  const fetchGames = async () => {
    try {
      ...
      const gamesResponse = await $gameService.getGames({
        ...
        provider: getOriginProviderCode(provider.value),
        page: page.value,
        limit,
      });
      ...
      totalLobbyGame.value = gamesResponse.total_game_lobby;
  ...
  const countPlaceholders = (itemPerRow: number) => {
    if (!games.value || games.value.length < totalItems.value) return 0;
    const remainder = totalItems.value % itemPerRow;
    return remainder ? itemPerRow - remainder : 0;
  };
