  ...
  <DialogPortal>
    ...
    <DialogContent
      ...
      :class="
        cn(
          ...
          props.type === 'hybrid' && isMobileOrTablet ? 'min-h-dvh min-w-full rounded-none' : 'rounded-lg',
          props.class,
        )
      "
      @interact-outside="
    ...
    >
      ...
      <DialogClose
        :class="
          cn(
            'ring-offset-background absolute right-6 top-0 h-[60px] rounded-sm opacity-100 transition-opacity hover:opacity-70 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground',
            props.closeClass,
          )
        "
      >
