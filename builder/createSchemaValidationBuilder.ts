  ...
  return {
    ...
    buildUsernameSchema() {
      schema.username = z
        ...
        .min(validate.username.min, {
          message: `Vui lòng nhập hơn ${validate.username.min} ký tự`,
        })
        .max(validate.username.max, {
          message: `Vui lòng không nhập hơn ${validate.username.max} ký tự`,
        })
        .regex(/^[a-zA-Z0-9]+$/g, {
      ...
      schema.phone = z.preprocess(
        ...
        z
          ...
          .min(validate.phone.length, {
            message: `Vui lòng không nhập ít hơn ${validate.phone.length} chữ số`,
          })
